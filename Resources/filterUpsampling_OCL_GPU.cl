...
__kernel void filterUpsamplingGPU_kernel(__global float* restrict iD, __global float* restrict oD, __global float* restrict zD, int height, int sizeKernel, int width);
__kernel void filterUpsamplingGPU_kernel(__global float* restrict iD, __global float* restrict oD, __global float* restrict zD, int height, int sizeKernel, int width) {
  float filterUp[1];
  sizeKernel = 53;
  if ((convert_int(get_group_id(0))) < (height >> 5)) {
    filterUp[0] = 0.000000e+00f;
    for (int u = 0; u < (((sizeKernel / 2) * 2) + 1); ++u) {
      if ((((convert_int(get_group_id(1))) * 32) + (convert_int(get_local_id(1)))) < width) {
        filterUp[0] = (filterUp[0] + (iD[((((((convert_int(get_group_id(1))) * 32) + ((((convert_int(get_group_id(0))) * 32) + (convert_int(get_local_id(0)))) * width)) + (convert_int(get_local_id(1)))) + u) - (sizeKernel / 2))] * zD[u]));
      }
    }
    if ((((convert_int(get_group_id(1))) * 32) + (convert_int(get_local_id(1)))) < width) {
    ...
    filterUp[0] = 0.000000e+00f;
    for (int u_1 = 0; u_1 < (((sizeKernel / 2) * 2) + 1); ++u_1) {
      if ((((convert_int(get_group_id(0))) * 32) + (convert_int(get_local_id(0)))) < height) {
        if ((((convert_int(get_group_id(1))) * 32) + (convert_int(get_local_id(1)))) < width) {
          filterUp[0] = (filterUp[0] + (iD[((((((convert_int(get_group_id(1))) * 32) + ((((convert_int(get_group_id(0))) * 32) + (convert_int(get_local_id(0)))) * width)) + (convert_int(get_local_id(1)))) + u_1) - (sizeKernel / 2))] * zD[u_1]));
        }
      }
    }
    if ((((convert_int(get_group_id(0))) * 32) + (convert_int(get_local_id(0)))) < height) {
